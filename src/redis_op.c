...
int rop_set_key_value(redisContext *conn, const char *key, const char *values)
{
    int ret = 1;
    redisReply *reply = NULL;

    reply = redisCommand(conn, "SET %s %s", key, values);
    rop_test_reply_type(reply);

    if (strcmp(reply->str, "OK") != 0)
    {
        ret = 0;
        LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "SET %s %s error %s\n", key, values,conn->errstr);
        goto End;
    }
End:
...
int rop_get_key(redisContext *conn, char *key, char *values)
{
    int ret = 1;
    redisReply *reply = NULL;
    ...
    {
        ret = 0;
        LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "GET %s  error %s\n", key,conn->errstr);
        goto End;
    }
    strncpy(values, reply->str, reply->len);
    values[reply->len] = '\0';
